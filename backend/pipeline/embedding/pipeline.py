...
def _cluster_date_group(rows: list[dict], next_cluster_id: int) -> tuple[list[dict], int]:
    ...
    - Removed the "only 1 source → skip clustering" guard.  Every article,
      even from a single source, must receive a cluster_id (requirement).
    - min_cluster_size is forced to 1 so single-article clusters are NOT
      dissolved.  Each article is guaranteed a cluster.
    - Tag key fixed: records use "tags" (list); cluster_tag is derived from
      that list and stored separately as "cluster_tag_str" for the DB write.
    ...
    records : list[dict]
        ...
        - cluster_id (int, always set — never None)
        - cluster_label (int)
        - cluster_tag_str (str, canonical tag for the cluster)
    next_cluster_id : int
    ...
    for row in rows:
        ...
        # Derive a single canonical tag string from the tags list for labelling
        ...
        records.append(
            {
                ...
                "cluster_tag_str": None,   # filled after clustering
    ...
    # ── Cluster with min_cluster_size=1 so every article gets a cluster ────────
    ...
    labels = cluster_articles(
        ...
        min_cluster_size=1,          # BUG FIX: never dissolve single-article clusters
    )
    ...
    # Derive cluster_tag_str for each record from its tags list
    # (most frequent tag in the cluster, falls back to first tag, then "untagged")
    cluster_tags: dict[int, list[str]] = {}
    for r in records:
        cid = r["cluster_id"]
        cluster_tags.setdefault(cid, [])
        cluster_tags[cid].extend([t for t in (r["tags"] or []) if t])

    cluster_best_tag: dict[int, str] = {}
    for cid, tag_list in cluster_tags.items():
        if tag_list:
            from collections import Counter as _Counter
            cnt = _Counter(tag_list)
            cluster_best_tag[cid] = cnt.most_common(1)[0][0]
        else:
            cluster_best_tag[cid] = "untagged"

    for r in records:
        r["cluster_tag_str"] = cluster_best_tag.get(r["cluster_id"], "untagged")
...
def run_pipeline(process_date: date | str | None = None) -> dict:
    ...
    # Seed next_cluster_id from the DB so IDs are always unique across runs.
    # We also add a large day-based offset (YYYYMMDD * 100000) so cluster IDs
    # are visually traceable to the day they were created.
    today_offset = int(datetime.utcnow().strftime("%Y%m%d")) * 100_000
    db_max = get_max_cluster_id() or 0
    next_cluster_id = max(today_offset, db_max + 1)
